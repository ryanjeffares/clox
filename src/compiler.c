...
static void endScope() {
    ...
    while (current->localCount > 0 && 
        ...
        printf(local->final ? "Local final: " : "Local var: ");
        for (int i = 0; i < local->name.length; i++) {
            printf("%c", local->name.start[i]);
        }
        printf("\n");
#endif
        emitByte(OP_POP);
...
static void parsePrecedence(Precedence precedence, bool reassignment) {
    advance();
    ...
    if (reassignment) {
        ...
        // find local with matching name and check if it is final
        for (int i = current->localCount - 1; i >= 0; i--) {
            if (identifiersEqual(target, &current->locals[i].name)) {
                if (current->locals[i].final) {
                    error("Cannot reassign to final.");
                    return;
                }
            }
        }
...
static void defineVariable(uint8_t global, bool finalGlobal) {
    if (current->scopeDepth > 0) {
        markInitialized();
        return;
    }

    emitBytes(finalGlobal ? OP_DEFINE_GLOBAL_FINAL : OP_DEFINE_GLOBAL, global);
}
...
static void expression(bool reassignment) {
    parsePrecedence(PREC_ASSIGNMENT, reassignment);
}
...
static void function(FunctionType type) {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            ...
            // if (check(TOKEN_FINAL)) {
            //     consume(TOKEN_FINAL, "");  // consume 'final'
            //     uint8_t constant = parseVariable("Expected parameter name.", true);
            // } else {
            uint8_t constant = parseVariable("Expected parameter name.", false);
...
static void varDeclaration() {
    uint8_t global = parseVariable("Expected variable name.", false);
...
static void finalDeclaration() {
    uint8_t global = parseVariable("Expected final name.", true);

    if (!match(TOKEN_EQUAL)) {
        error("'final' must be assigned to upon declaration.");
        return;
    }
...
static void forStatement() {
    ...
    if (!match(TOKEN_SEMICOLON)) {
        expression(false);
        consume(TOKEN_SEMICOLON, "Expected semicolon after loop condition.");
...
static void returnStatement() {
    if (current->type == TYPE_SCRIPT) {
        if (!match(TOKEN_NUMBER)) {
            error("Can only return numeric exit code from top level.");
        }
        number(false);
        consume(TOKEN_SEMICOLON, "Expected semicolon after return value.");
        emitByte(OP_RETURN);
        return;
    }
